...
float xDistanceTravelled = 0.0f, yDistanceTravelled = 0.0f;
        ...
        if (pc.readable() && pc.read(&buf, sizeof(buf)))
        {
            if (buf == 's' && jidou == 0) //  /henkouten1
            {
                ...
                while (xDistanceTravelled < targetXDistance && yDistanceTravelled < targetYDistance)
                {
                    ...
                    xDistanceTravelled = penguin.receive[1].enc;
                    yDistanceTravelled = penguin.receive[1].enc;
                    ...
                    if (output_x + output_y > n)
                    {
                        penguin.pwm[1] = n;
                    }
                    else
                    {
                        penguin.pwm[1] = output_x + output_y;
                    }
